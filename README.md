# Fast Singular Value Decomposition for 3x3 matrices
...
## Performance
A benchmark program is included that allows you to measure the performance of SVD on your own machine. Run it from ``build`` with

``./svd_bench``

On my 3.2 GHz Intel Core i9-12900K, the benchmark showed that a 3×3 double precision SVD took 0.2662 microseconds (μs) on average.
This is only 1.14× slower than Eric Jang's [3×3 float SVD](https://github.com/ericjang/svd3), which implements the method of [McAdams et al.](https://minds.wisconsin.edu/handle/1793/60736)
<br>Float 3×3 SVD itself took 0.2397 μs, roughly the same as Eric’s implementation.
